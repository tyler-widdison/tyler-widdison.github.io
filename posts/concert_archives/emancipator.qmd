...
For this post, I focus on data from <https://www.concertarchives.org/bands/emancipator>. This site had the most complete concert history I could find.

Regarding the data, there are some issues with when there is a festival type. The festivals will repeat multiple times. Festivals are an issue when putting this data into a tidy format.
...
This still returns some problematic rows. From here manual intervention is needed to remove the concerts that didn't exist. I'll load my clean data I worked on manually.
...
After gathering the geo data from the Location (City, State, Country) I add which album is most recent to each date. So we can look into touring after an album release.
